...
R_color = np.array([1, 0, 0, 0.6])
G_color = np.array([0, 1, 0, 0.6])
B_color = np.array([0, 0, 1, 0.6])
P_color = np.array([100/255, 0, 100/255, 0.6])

colors = [R_color,G_color, B_color, P_color]
def show_mask(mask, ax, gt_class=None, GT=False):
    if type(mask) is torch.Tensor:
        mask = mask.detach().cpu().numpy()
    
    color = colors[gt_class]
